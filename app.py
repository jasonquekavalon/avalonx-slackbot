...
def slack_gcp():
    ...
    if (msg_validation(req)):
        ...
        if "message_id" not in req['text']:
            ...
            make_response(message + f"Your Message ID is {message_id}. To check the status of your message, type `/avalonx-message-status {message_id}`.", 200)
        else:
            message_id = req['text'].split()[1] #/avalonx message_id: 1283219837857402 <message>
            make_response(message,200)
